...
for root,dirs,files in os.walk(args.target_path):
    for path in files:
        ...
        try:
            ...
            continue
        # extract printable section from the target sample
        ...
        for section in sections:
            #sectionName = str(section.Name,'utf-8')
            #print(sectionName)
            network.add_node(section.Name,label=section.Name,color='blue',
               penwidth=10,bipartite=1)
            network.add_edge(section.Name,path,penwidth=2)
        if sections:
